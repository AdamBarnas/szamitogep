...
def choose_path(mat, lista, rozw, lr=None, lc=None):
    ...
    for i in range(n):  # Dla każdego rzędu
        for j in range(m):  # Dla każdej kolumny
            if mat[i][
                j] == 0:  # Jeśli znajdę w macierzy zero to dla tej kolumny i tego wiersza znajduję najmniejsze wartosći
                ...
                d[(i, j)] = max(mr,
                                mc)  # W słownikach zapisuję odpowiednie wierzchołki oraz wyliczoną dla nich wartość najmniejszą
    ...
    if n <= 2:  # Zabezpieczenie przed małymi macierzami - przy końcowym rezultacie, ostatecznie musimy dodać tylko jeden punkt
        rozw.append(list(dk.keys())[
                        1])  # Gdyż mamy tylko jedną kolumnę i wierzchołek, który pozostał, jeśli rozważamy symetryczną macierz
...
#     [1, 2, 4, 1, 3, 2, inf]
# ])
matrix = np.array([[float("inf"), 1, 0, 2, 2],
                   [3, float("inf"), 2, 0, 1],
                   [1, 2, float("inf"), 2, 0],
                   [4, 0, 3, float("inf"), 4],
                   [0, 2, 7, 0, float("inf")]])
